...
import machineLearning.MLModels.RandomForestRegressionModel
...
class RandomForestRegressionModelTest extends FunSuite{
  test("MLModels.RandomForestRegressionModel.randomForestRegressionModel") {
    ...
    assert(
      RandomForestRegressionModel.randomForestRegressionModel(df_train, df_test, evaluator)._1.transform(test_10).select("prediction").first().get(0) !== RandomForestRegressionModel.randomForestRegressionModel(df_train, df_test, evaluator)._1.transform(test_20).select("prediction").first().get(0)
    )
  }
}
