...
import machineLearning.MLModels.LinearRegressionModel
...
class LinearRegressionModelTest extends FunSuite{
  test("MLModels.LinearRegressionModel.linearRegressionModel") {
    ...
    assert(
      LinearRegressionModel.linearRegressionModel(df_train, df_test, evaluator)._1.transform(test_10).select("prediction").first().get(0) !== LinearRegressionModel.linearRegressionModel(df_train, df_test, evaluator)._1.transform(test_20).select("prediction").first().get(0)
    )
  }
