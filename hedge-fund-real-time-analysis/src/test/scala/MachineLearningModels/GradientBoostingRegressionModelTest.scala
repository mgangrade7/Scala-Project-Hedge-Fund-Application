...
import machineLearning.MLModels.GradientBoostingRegressionModel
...
class GradientBoostingRegressionModelTest extends FunSuite {
  test("MLModels.GradientBoostingRegressionModel.gradientBoostingRegressionModel") {
    ...
    assert(
      GradientBoostingRegressionModel.gradientBoostingRegressionModel(df_train, df_test, evaluator)._1.transform(test_10).select("prediction").first().get(0) !== GradientBoostingRegressionModel.gradientBoostingRegressionModel(df_train, df_test, evaluator)._1.transform(test_20).select("prediction").first().get(0)
    )
  }
